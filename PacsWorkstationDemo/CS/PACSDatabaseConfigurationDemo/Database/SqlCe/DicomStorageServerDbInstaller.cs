...
{
   class DicomStorageServerSqlCeInstaller
   {
      public static void InstallDatabase(string connectionString)
      {
         Stream stream;

         stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(Constants.StorageServerDb);
      ...
      {
         public const string StorageServerDb = "CSPacsDatabaseConfigurationDemo.Database.SqlCe.StorageServer.sdf";
      }
   }
}
